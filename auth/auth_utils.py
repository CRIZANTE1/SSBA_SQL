...
def get_user_permissions() -> pd.DataFrame:
    ...
    try:
        ...
        if 'email' not in df.columns or 'role' not in df.columns:
            st.error("A aba 'usuarios' na sua planilha precisa ter as colunas 'email' e 'role'.")
            return pd.DataFrame(columns=expected_cols)

        df['email'] = df['email'].str.lower().str.strip()
        df['role'] = df['role'].str.lower().str.strip()
        return df[['email', 'role']]
...
def get_user_email() -> str | None:
    ...
    if is_user_logged_in() and hasattr(st, 'email'):
...
def get_user_role() -> str:
    ...
    if permissions_df.empty:
        st.error(f"Acesso negado. Nenhuma permissão configurada ou seu e-mail ({user_email}) não está na lista de usuários autorizados.")
        st.stop()
